...
let URL = "https://teachablemachine.withgoogle.com/models/Jab2eClD4/";
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 0.04 && time <= 0.05 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 0.07 && time <= 0.08 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if ((time >= 0.09 && time <= 0.10 && !poseState.firstWindowTriggered) ||
            ...
            case '4':
                ...
                break;
            case '5':
                if (time >= 0.18 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/Jab2eClD4/");
    }
}
